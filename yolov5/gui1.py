...
class Example(QWidget):
    ...
    @torch.no_grad()
    def run(self,weights=r"C:\Users\R\Desktop\helmet\content\yolov5\runs\train\gun_yolov5s_result\weights\best.pt",  # model.pt path(s)
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if view_img:
                    im0 = annotator.result()
                    cam = cv2.cvtColor(im0,cv2.COLOR_BGR2RGB)
                    img = QImage(cam,cam.shape[1],cam.shape[0],QImage.Format_RGB888)
                    pix=QPixmap(img)
                    self.frame.setPixmap(pix)
                    
                    if self.btn_off.isChecked()==True:
                        break
